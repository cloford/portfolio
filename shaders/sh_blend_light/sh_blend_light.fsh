...
// これをコメントアウトして、ライトフラグメントのブレンドを無効にします。
#define BLEND_LIT_FRAGMENTS
// これをコメントアウトして、非点灯のフラグメントのブレンドを無効にします。
#define BLEND_UNLIT_FRAGMENTS

// ブレンド時にこのフラグメントから分岐させるテキスト数は？
...
uniform bool u_AttenuationEnabled;	// 減衰を有効にしていますか？
...
// Light intensity to a point emitter P // 点発光体への光量 P
...
// 0 から 1 の間の指定された x の強度の減衰量を返します。
// この関数のパラメータ設定はここで行うことができます https://www.desmos.com/calculator/nmnaud1hrw
// 光の強さのためにLUTを使用したくない場合（つまり、このシェーダを修正してその機能を削除した場合）。
// この関数よりも別の減衰関数（ゼロに収束するもの）が必要になるでしょう。
