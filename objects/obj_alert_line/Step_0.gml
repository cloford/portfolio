...
if(cast_count <= 0)
{
	...
	//shieldに触れたとき
	if(touch_to_shield != noone)
	{
		touch_to_shield.damage = damage;
		instance_destroy();
	}
	
	//actorに触れたとき
	var ip_actor = instance_place(x, y, obj_parent_actor);
	if(ip_actor != noone) and (ip_actor != creator) //発射した主体でないなら
	{
		ip_actor.damage = damage;
		instance_destroy();
	}
}